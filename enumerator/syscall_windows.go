...
func setupDiGetClassDevs(guid *guid, enumerator *string, hwndParent uintptr, flags uint32) (set hDevInfo, err error) {
...
func setupDiGetDeviceRegistryProperty(set hDevInfo, devInfo *pspDevInfoData, prop deviceProperty, propType *uint32, propBuf *byte, propBufSize uint32, reqSize *uint32) (res bool) {
	r0, _, _ := syscall.Syscall9(procSetupDiGetDeviceRegistryPropertyW.Addr(), 7, uintptr(set), uintptr(unsafe.Pointer(devInfo)), uintptr(prop), uintptr(unsafe.Pointer(propType)), uintptr(unsafe.Pointer(propBuf)), uintptr(propBufSize), uintptr(unsafe.Pointer(reqSize)), 0, 0)
	res = r0 != 0
	return
}
