...
func (e *C.io_registry_entry_t) GetClass() string {
	obj := (*C.io_object_t)(e)
	return obj.GetClass()
}
...
func (it *C.io_iterator_t) IsValid() bool {
	return C.IOIteratorIsValid(*it) == C.true
}

func (it *C.io_iterator_t) Reset() {
	C.IOIteratorReset(*it)
}

func (it *C.io_iterator_t) Next() (C.io_object_t, bool) {
	res := C.IOIteratorNext(*it)
	return res, res != 0
}

func (it *C.io_iterator_t) Release() {
	C.IOObjectRelease(C.io_object_t(*it))
}

// io_object_t

func (obj *C.io_object_t) Release() {
	C.IOObjectRelease(*obj)
}

func (obj *C.io_object_t) GetClass() string {
	class := make([]C.char, 1024)
	C.IOObjectGetClass(*obj, &class[0])
